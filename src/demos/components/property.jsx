...
const Property = (Props) => {
  ...
  Object.keys(propertyJSON).forEach((properties) => {
    if (properties === Props.name) {
      if (propertyJSON[properties].length === undefined) {
        ...
        propertyJSON[properties].forEach(element =>
          (
  ...
  if (bodysingle.length === 0) {
    return (
      bodymultipleprops.map(elements => (
        <Grid key={elements}>
          <Inner>
          ...
          {elements.map(element => (
            <Inner key={element} style={{ borderBottom: "solid black 1px", marginBottom: "16px", paddingTop: "5px" }}>
